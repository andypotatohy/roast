function [s,ds1,ds2,ds3] = spm_sample_priors8_noBspline(tpm,x1,x2,x3)
% Sample prior probability maps without any B-spline
% FORMAT [s,ds1,ds2,ds3] = spm_sample_priors8_noBspline(tpm,x1,x2,x3)
...
if nargout<=1,
    tot = zeros(dx);
    for k=1:Kb,
%         a    = spm_bsplins(tpm.dat{k},x1,x2,x3,[deg deg deg  0 0 0]); % sampling TPM using B-spline interpolation
%                                                 % deg is NOT same as used by spm_bsplinc???
%         s{k} = ones(dx)*tpm.bg2(k);
%         s{k}(msk1) = exp(a);
        s{k}(msk1) = tpm.dat{k}(x1,x2,x3); % this is not correct
%         s{k}(msk2) = tpm.bg1(k);
        tot  = tot + s{k};
    end
%     msk      = ~isfinite(tot);
%     tot(msk) = 1;
    for k=1:Kb,
%         s{k}(msk) = tpm.bg2(k);
        s{k}      = s{k}./tot; % normalize the loaded TPM
    end
else
    ds1 = cell(1,Kb);
    ds2 = cell(1,Kb);
