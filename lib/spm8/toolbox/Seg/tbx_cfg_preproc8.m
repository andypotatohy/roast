...
% Modified by Yu (Andy) Huang on 2013-05-02
...
% ---------------------------------------------------------------------
% mrfMode Switch to run different algorithms
% ---------------------------------------------------------------------
mrfMode         = cfg_entry;
mrfMode.tag     = 'mrfMode';
mrfMode.name    = 'MRF running mode';
mrfMode.help    = {'A switch to run different algorithms: default New Segment (0), New Segment with MRF cleanup (1), and MRF-based EM iteration (2).'};
mrfMode.strtype = 'e';
mrfMode.num     = [1 1];
mrfMode.val     = {0}; % {2};
 % ANDY 2013-05-02
...
tcm.help    = {'Select the Tissue Correlation Map (TCM) for MRF-based algorithm to run. It can be a global TCM (K x K matrix) or local TCM (dim1 x dim2 x dim3 x K x K x 6 matrix). Leave this option blank if you do not want to run MRF algorithm and just need to run default New Segment.'};
tcm.filter = '.mat';
tcm.ufilter = '.*';
tcm.num     = [1 1];
...
warp.val     = {mrfMode tcm reg affreg samp write }; % ANDY 2013-05-02
warp.help    = {
