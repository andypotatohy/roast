...
%   delta=(maxval-minval)/64;
  ...
  for i=1:length(intx)
      mask_elec = mask_elec + (sqrt((Xi-inty(i)).^2 + (Yi-intx(i)).^2) <= 0.02); % check if grid point is close enough to electrode i
                                                                          % this 0.02 is for adjusting the radius of each electrode (originally 0.03)
                                                                          % no option in mytopoplot for this purpose
                                                                          % ANDY 2014-09-17
  end
...
%   colormap(cm);
  ...
  mask_head = ( sqrt(Xi.^2 + Yi.^2) <= 0.355   ) + (mask_elec~=0)  ; % mask outside the plotting circle
  ii = find(mask_head == 0);
  ...
  caxis([floor(amin) ceil(amax)]) % set coloraxis
%   caxis([-30 30]);
%   caxis([minval-delta;maxval]);  
else % if STYLE 'blank'
%
%%%%%%%%%%%%%%%%%%%%%%% Draw blank head %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
